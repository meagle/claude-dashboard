...
**Goal:** Make a bursty, multi-agent fleet scannable at a glance — surface *which* agent each card is, group cards by repo when the list gets crowded, and add cheap, robust actions — plus fold in the low-risk cleanups deferred from Part A.
...
- **Repo grouping is bursty-aware:** auto-flatten to a plain list below ~6 sessions; only group when crowded. Repo is the primary grouping key; agent is a secondary toggle.
...
**Interfaces produced:**
- `SOURCE_META: Record<string, { displayName: string; color: string; iconKey: string }>` — Node-free (no fs/electron imports), the single source of agent display identity for BOTH the descriptors and the renderer.
- `sourceDisplayName(source: string): string` — safe display name with a sensible fallback.
- `SOURCE_BY_ID` / a `toSource(id): Session['source']` helper that fails to compile if an id isn't in the union.

- [ ] **Step 1:** Extract the `{displayName, color, iconKey}` currently inline on each descriptor into `SOURCE_META` (keyed by id), plus the `desktop` presence entry. Descriptors and `PRESENCE_SOURCES` reference `SOURCE_META` so identity is defined once.
- [ ] **Step 2:** Add `sourceDisplayName(id)` (returns `SOURCE_META[id]?.displayName ?? 'Agent'`). Re-export both from `typesEntry.ts` (confirm they stay Node-free — a plain object + function, no fs).
- [ ] **Step 3:** Remove the dead `payload` field from `AgentDescriptor` and the three descriptors; confirm nothing reads it (grep) and the build stays green.
- [ ] **Step 4:** Replace hook.ts's `agent.id as Session['source']` casts with the checked helper; add the `ids ⊆ union` test so a future out-of-union id fails CI instead of silently writing a bad `source`.
...
## Task 3: Collapsible repo grouping (bursty auto-flatten)

**Files:**
- Create: `packages/menubar/src/renderer/utils/grouping.ts` (+ test) — `groupByRepo(sessions, threshold=6)`.
- Create: `packages/menubar/src/renderer/components/SessionGroup.tsx` — collapsible header.
- Modify: `SessionList.tsx` — flat below threshold, grouped above; secondary group-by-agent toggle.

**Interface:** `groupByRepo(sessions, threshold=6) → { flat: boolean; groups: { key, label, sessions }[] }`. Repo key = `dirName` (fall back to `workingDir`); desktop presence excluded from grouping; `flat: true` below threshold.

- [ ] **Step 1:** `grouping.ts` per the interface; tests for flat-below-threshold, grouping at/above, desktop excluded.
- [ ] **Step 2:** `SessionGroup.tsx` — header shows repo label, per-agent colored dots (unique sources in the group), and a "N waiting" count when any session is `waiting_*`; collapse state persisted in `localStorage` keyed by `group:<key>`.
- [ ] **Step 3:** Wire into `SessionList`: flat path unchanged when `flat`; else render groups (keep the `AnimatePresence` card animation inside each). Desktop card stays appended once at the bottom. Add a secondary "group by agent" toggle.
- [ ] **Step 4:** Tests for crowded (headers appear) vs few (flat); run Vitest + build; commit.

---

## Task 4: Cheap-control actions (robust only)
...
- [ ] **Step 1:** Make notifications actionable — `n.on('click', () => focusTerminal(s.pid, s.termSessionId))`.
...
- [ ] Real-app verification via the `run` skill (drive the actual Electron app; screenshot cards showing agent chips + a grouped, crowded state) — browser-Playwright can't render this app (renderer needs Electron IPC).
- [ ] README: refresh the per-agent card-field table + document chips, grouping, and the reveal/open actions.
...
- Grouping auto-flattens below threshold; desktop presence excluded.
