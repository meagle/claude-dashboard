...
const workers = [1, 2, 3].map(() =>
  spawn(process.execPath, [WORKER_SCRIPT], { detached: false, stdio: 'ignore' })
);
...
async function run() {
  ...
  let backup = null;
  try { backup = fs.readFileSync(SESSIONS_FILE, 'utf8'); } catch {}
  ...
  const s1 = session({
    id: 1, dir: 'claude-dashboard', branch: 'master',
    ...
    tasks: [
      ...
      { id: 't2', subject: 'Add streaming response field', status: 'in_progress' },
  ...
  const s2 = session({
    id: 2, dir: 'api-service', branch: 'feature/auth',
  ...
  const s3 = session({
    id: 3, dir: 'payments-service', branch: 'main', worktree: 'stripe-v2',
    ...
    extra: {
      ...
      partialResponse: 'The Stripe v2 API uses a different authentication flow…',
    },
  });
  write([s1, s2, s3]);
  await sleep(2500);
  ...
  const s2_input = {
    ...
    currentTool: null,
    lastMessage: 'Should I apply rate limiting to internal health-check endpoints too?',
    lastActivity: Date.now(),
  };
  ...
  const s2_done = {
    ...
    status: 'done',
    currentTool: null,
    lastMessage: 'Rate limiting applied to all public endpoints. Health checks excluded.',
    lastActivity: Date.now(),
    costUsd: 0.0189,
    turns: 5,
    contextPct: 14,
